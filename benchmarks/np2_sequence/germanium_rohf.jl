...
"""
    solve_germanium_rohf(R_max; estado, use_diis, tol, max_iter, diis_thresh, save, alpha_d, r_c)
...
`germanium_rohf_vpol.jl`, que no tiene C-DIIS y cuya rama 3P tenia invertido el signo de f_2. Con `alpha_d = 0`, el valor por defecto, el
...
function solve_germanium_rohf(R_max; verbose::Bool=true, estado=nothing,
    ...
    for iter in 1:max_iter
        ...
        build_specific_K_matrix!(ws, K_4p_intra, orbitals[8], 2, coeff_k2)
        ...
        # (2p, 3p), la d de core (3d) y el 4p abierto con su ocupacion fraccionaria.
        # El historial guarda las Fock CRUDAS: el level shift de abajo se aplica
        # despues, y solo mientras DIIS siga apagado.
        F_use = Matrix{Float64}[F_s, F_core_p, F_3d, F_4p]
        if use_diis
            D_s  = block_density(orbitals, (1, 2, 4, 7), active_s)
            D_cp = block_density(orbitals, (3, 5), active_p)
            D_3d = block_density(orbitals, (6,), active_d)
            D_4p = block_density(orbitals, (8,), active_p)
            # El espacio ocupado l=1 incluye al 4p abierto: es contra el que se
            # ortogonaliza el core y de donde salen los multiplicadores de Lagrange.
            C_s  = occ_block(orbitals, (1, 2, 4, 7), active_s)
            C_p  = occ_block(orbitals, (3, 5, 8), active_p)
            C_d  = occ_block(orbitals, (6,), active_d)
            F_use, diis_on = diis_update!(diis, iter,
                                          Matrix{Float64}[F_s, F_core_p, F_3d, F_4p],
                                          Matrix{Float64}[D_s, D_cp, D_3d, D_4p],
                                          Matrix{Float64}[C_s, C_p, C_d, C_p],
                                          UnitRange{Int}[active_s, active_p, active_d, active_p],
                                          ws.S)
        end
        F_s_use, F_cp_use, F_3d_use, F_4p_use = F_use[1], F_use[2], F_use[3], F_use[4]

        # ==========================================
        # PHASE 4: Scheduled Dynamic Level Shifting
        # ==========================================
        # Un desplazamiento constante que nunca se apaga amortigua la convergencia de
        # forma permanente. Con C-DIIS el shift es solo el regimen de arranque: se
        # suelta en cuanto el residual del conmutador entra en la cuenca de atraccion.
        ...
        if level_shift > 0.0
            ...
            F_3d_use = copy(F_3d_use); F_4p_use = copy(F_4p_use)

            # Apply shift to the virtual space
            F_s_use  .+= level_shift .* (ws.S .- S_Ps_S)
            F_cp_use .+= level_shift .* (ws.S .- S_Pp_S)
            F_3d_use .+= level_shift .* (ws.S .- S_Pd_S)
            F_4p_use .+= level_shift .* (ws.S .- S_Pp_S)
        ...
        evals_f3d, evecs_f3d = eigen(Symmetric(F_3d_use[active_d, active_d]), ws.S[active_d, active_d])
        evals_f4p, evecs_f4p = eigen(Symmetric(F_4p_use[active_p, active_p]), ws.S[active_p, active_p])
        ...
        if delta < tol && (!diis.on || diis.frozen || diis.stall == 0)
            ...
            filename = joinpath(@__DIR__, "germanium_rohf_results_$(estado)_R$(R_max)$(sufijo).jld2")
    ...
    @warn "Germanio: no convergio en $max_iter iteraciones (tol = $tol)."
