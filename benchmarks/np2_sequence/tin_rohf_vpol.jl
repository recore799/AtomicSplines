...
function solve_tin_rohf_vpol(R_max, alpha_d, r_c, estado; verbose::Bool=true)
    if estado ∉ ["av", "3P"]
    ...
    println("=== Tin ROHF Optimización V_pol: $estado (Z=50) ===")
    @printf("  alpha_d = %.3f, r_c = %.3f\n", alpha_d, r_c)
    
    N_elems = 500
    Z = 50.0

    ws = cached_init_scf_workspace(R_max, N_elems, Val(8), Z; γ=4.0, calc_R_matrices=true, alpha_d=alpha_d, r_c=r_c)
    basis = ws.basis
    ...
    for iter in 1:1000
        ...
        if level_shift > 0.0
            ...
            F_core_d .+= level_shift .* (ws.S .- S_Pd_S)
            F_5p     .+= level_shift .* (ws.S .- S_Pp_S)
        end
        ...
        evals_fcd, evecs_fcd = eigen(Symmetric(F_core_d[active_d, active_d]), ws.S[active_d, active_d])
        evals_f5p, evecs_f5p = eigen(Symmetric(F_5p[active_p, active_p]), ws.S[active_p, active_p])
...
solve_tin_rohf_vpol(30.0, 1.0, 1.0, "3P")
