...
Se destacan tres aspectos de eficiencia en este diseño, todos ellos internos al bucle de ensamblaje; la organización de más alto nivel se resume al cierre del apéndice:
...
Las condiciones de frontera se imponen sin tocar el vector fuente. La del origen se aplica restringiendo el sistema a los splines de índice mayor que $k+1$, y la del extremo exterior sumando el término de Robin de la Ecuación \eqref{eq:bc-robin} al último elemento diagonal de la matriz del operador, una sola vez al construir la factorización. Como ninguna de las dos altera la simetría ni el carácter definido positivo, la descomposición de Cholesky se calcula una vez por multipolo y se reutiliza durante todo el ciclo, y el sistema resuelto es menor que el original en lugar de mayor.
...
El resultado es un solucionador donde el ciclo autoconsistente está dominado casi exclusivamente por operaciones BLAS de niveles 2 y 3. La distinción entre los tres niveles de esa biblioteca es la razón por la que esto importa, y se refiere a la relación entre el trabajo aritmético que realiza una rutina y el volumen de datos que debe mover desde memoria. Las de nivel 1 operan entre vectores y ejecutan del orden de $n$ operaciones sobre $n$ datos; las de nivel 2 son de matriz por vector, con $n^2$ operaciones sobre $n^2$ datos; y las de nivel 3 son de matriz por matriz, con $n^3$ operaciones sobre $n^2$ datos. Solo en este último caso el número de operaciones crece más rápido que el de accesos a memoria, lo que permite reutilizar cada dato cargado en caché muchas veces y acercarse al rendimiento máximo del procesador. Las de nivel 1 están limitadas por el ancho de banda de memoria y no por la capacidad de cálculo.

Conviene señalar una decisión de implementación relacionada. Aunque el ensamblaje por elementos produce de manera natural matrices de banda, el motor no las almacena en una estructura de datos especializada para ese patrón, sino en arreglos densos contiguos, por las razones de sobrecosto que se discuten en la Sección \ref{sec:repositorio}. La estructura de banda se aprovecha entonces a través de las rutinas de factorización y no del formato de almacenamiento, lo que resulta ventajoso para los tamaños de malla empleados aquí. Esta infraestructura permite alcanzar la precisión numérica característica de los B-splines con un costo que escala favorablemente, y establece la base sobre la que se construyen los cálculos de interacción de configuraciones y de estructura fina presentados en este trabajo.
