...
\todo{el cuadro se sale un poco de la pagina}
\begin{table}[htbp]
    \centering
...
\todo[inline]{me gustaria decir algo como que la expansion claramente debe converger uniformemente, por lo que la sumatoria conmuta con el signo integral, pero creo que eso es para series, y como esta es una sumatoria finita, esto se cumple en automatico?.}
...
\todo{cabe anadir aqui que entonces, aunque dan el mismo resultado, nuestro metodo de galerkin nos dota de un manejo mas directo del algebra lineal que es mucho mas facil y rapido implementar en computadora que el metodo de ritz tradicional, o algo asi justificando la eleccion (ademas de los B-splines).}
...
\todo{esto lo habia hecho antes del apendice de tensores, ahora vamos a citar tensores mejor}
...
\todo{cita, creo que es al libro? o al articulo?}
...
\todo{bueno, aqui esta la referencia, tal vez hay que hacerla desde el principio una sola vez?}
Esta función $Y^k(a,b;r)$ encapsula toda la información necesaria para construir tanto los operadores de Coulomb $J$ (donde $a=b$) como los operadores de Intercambio $K$ (donde $a \neq b$) para cualquier átomo multielectrónico bajo la aproximación de campo central. Siguiendo el formalismo estándar de Hartree-Fock descrito por \textcite{fischer1977}, esta función permite además expresar las integrales de Slater de interacción directa ($F^k$) e intercambio ($G^k$) como simples valores esperados radiales, por ejemplo:
...
\todo{creo que estas condiciones de frontera estan mal, segun yo eran de Robin}
...
En consecuencia, en este trabajo adoptamos (\cref{eq:poisson-radial}) como la ecuación gobernante para determinar los potenciales de interacción, transformando el problema de Hartree-Fock en un sistema acoplado de ecuaciones diferenciales. A continuación demostramos formalmente la equivalencia entre esta forma diferencial y la definición integral (\cref{eq:Yk-final}).

\todo[inline]{creo que puedo incluir la derivacion de Poisson bajo Galerkin}
...
\todo{checar si esta expresion es correcta}
\[ \int_{a(r)}^{b(r)} \pdv{r}f(s) \dd{s} = 0 \]
...
En resumen, para resolver la \cref{eq:poisson-radial} numéricamente, imponemos:
\begin{equation}
\label{eq:bc-final}
