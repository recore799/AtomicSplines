...
Para comprender el origen de este error, debemos considerar la estructura de la capa cerrada sobre la cual opera el motor. Físicamente, una capa $p^6$ completamente llena posee $w=6$ electrones. La combinatoria de pares interactuantes de Coulomb para este sistema está dada por $\frac{w(w-1)}{2}$. Por lo tanto, la matriz angular del motor evalúa la repulsión de $\frac{6(5)}{2} = 15$ pares.
