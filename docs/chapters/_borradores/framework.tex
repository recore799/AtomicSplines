...
\item \textbf{Interacción de Intercambio (\(K_{ab}\)):}
  ...
  \end{equation}
  Este término surge exclusivamente si ambos electrones poseen el mismo estado de espín (\( \delta_{\sigma_a \sigma_b} = 1 \)). Es una consecuencia directa de la antisimetría exigida por el determinante de Slater y da origen al llamado hueco de Fermi, una zona de exclusión espacial alrededor de cada electrón que reduce eficazmente la repulsión electrostática entre partículas de espín paralelo.
\end{itemize}
...
Esto nos permite factorizar la integral de dos cuerpos en un término puramente radial y un factor angular:
...
Uniendo estas expresiones, obtenemos la fórmula explícita para evaluar los factores angulares \( c_k(a,b,c,d) \):
...
Las propiedades intrínsecas de los símbolos \( 3-j \) de Wigner imponen estrictas reglas de selección angulares que truncan de forma severa la sumatoria infinita sobre el orden multipolar \( k \):
...
donde \( c'_k(a,b,b,a) \) denota el factor angular específico para el intercambio cuántico esférico. Al promediar e incorporar estos operadores de campo medio sobre todas las capas ocupadas en el sistema, construimos el \textbf{Operador de Fock} radial general:
...
\item \textbf{El Bloque Fock de la Capa Abierta (\(F_p\)):}
  ...
  \end{equation}
  donde el potencial Coulombiano radial de la capa abierta \( J_{2p}^{intra} \) se escala exactamente por \( 1.0 \) en el monopolo fundamental \( k=0 \), y el término de intercambio cuántico intracapa \( K_{2p}^{intra} \) actúa selectivamente modulando la componente angular del multipolo cuadrangular \( k=2 \) mediante un factor de peso exacto de \( \frac{5}{25} \), reflejando de manera consistente la auto-interacción y correlación de espín restringida de la configuración abierta del Carbono.
\end{itemize}
