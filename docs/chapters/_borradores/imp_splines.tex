...
Sin embargo, observamos que los términos \( w_q \), \( r_q^{-1} \) y los tensores de colocación \( \bm{E} \) son invariantes. Podemos permutar las sumatorias para aislar los componentes dinámicos de los estáticos:

\begin{equation}
b_\mu = \sum_{\alpha, \beta} P_{\alpha \beta} \underbrace{ \left[ \sum_{q=1}^Q w_q \frac{E_{\mu q} E_{\alpha q} E_{\beta q}}{r_q} \right] }_{\text{Invariante de la malla}}
\end{equation}
...
\subsection{Implementación: Ensamblaje Estático Unificado}
...
Este procedimiento encapsula toda la discretización espacial de la base. Al finalizar esta rutina, el solucionador posee todos los ingredientes estáticos necesarios para iniciar el ciclo SCF sin volver a evaluar un solo polinomio.
...
\begin{itemize}
    ...
    \item A nivel \textbf{espacial}, la introducción de los tensores de colocación (\(\bm{E}\)) e interacción (\(\mathbf{W}\)) desacopla la discretización de la malla de la física del SCF.
\end{itemize}
