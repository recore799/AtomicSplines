...
\begin{enumerate}
\item \textbf{Micro-optimización (algorítmica):} La sustitución de la recursividad \textit{top-down} por un kernel de Programación Dinámica \textit{bottom-up}. Esto reduce la complejidad de la evaluación puntual de exponencial a cuadrática \( \order{k^2} \), explotando la localidad de los datos y eliminando la redundancia en el cálculo de sub-splines.
...
\subsection{Inversión del flujo de control}

Para evitar la redundancia y saturar la pila de ejecución (call stack), nuestra implementación adopta un esquema constructivo iterativo. Observemos que para cualquier punto \( r \in [t_{\mu}, t_{\mu + 1}) \), solo existen \( k \) funciones base con valor no nulo.

En lugar de calcular una sola función \( B_{i,k}(r) \), el kernel calcula simultáneamente el vector completo de las \( k \) funciones no nulas. El algoritmo procede iterativamente incrementando el orden del polinomio.

\begin{enumerate}
\item \textbf{Inicialización (orden 1):} Se identifica el índice \( \mu \) tal que \( t_{\mu} \leq r < t_{\mu + 1} \). El único valor no nulo es \( B_{\mu,1}(r) = 1 \).
\item \textbf{Inducción (orden \( j \to j+1 \)):} Se utilizan los \( j \) valores calculados en el paso anterior para generar los \( j+1 \) valores del nuevo orden.
\end{enumerate}

Este esquema transforma el árbol de recursión en una estructura triangular de dependencias. La complejidad computacional se reduce estrictamente a \( \order{k^2} \) operaciones de punto flotante, sin bifurcaciones condicionales complejas.
...
Al fijar nuestra atención en un elemento activo específico, la topología del problema se simplifica drásticamente. Sabemos, de manera determinista, que en cualquier intervalo \( [t_j, t_{j+1}) \) existen exactamente \( k \) funciones base con valor no nulo. Es decir, hay una segmentación espacial, que debemos considerar porque, aunque los B-splines son continuos globalmente hasta la derivada \( k-2 \) (\( C^{k-2} \)), las derivadas superiores presentan discontinuidades justo en los nudos.

Las reglas de cuadratura de alta precisión, como Gauss-Legendre, asumen que el integrando es una función suave (clase \( C^{\infty} \)) dentro del dominio de integración. Si intentáramos integrar a través de los nudos (ignorando la estructura de elementos), la presencia de estas discontinuidades en las derivadas destruiría la convergencia espectral del método, degradando el error a una tasa algebraica. Al restringir el dominio de integración a los límites del elemento \( [t_j,t_{j+1}) \), garantizamos que dentro de la región de evaluación, tanto la base como el potencial sean polinomios puros (analíticos). Esto permite explotar la exactitud teórica de la cuadratura gaussiana, obteniendo precisión de máquina con un número mínimo de puntos.
...
El término entre corchetes es independiente del estado del sistema, i.e. no cambia dentro del ciclo SCF. Definimos así el \textbf{Tensor de Interacción de Coulomb}, \( \mathbf{W} \):
...
Esta definición transforma el cuello de botella de la ecuación de Poisson. Ya no es necesario realizar integración numérica explícita durante el ciclo SCF; la construcción del término fuente se reduce a una contracción tensorial puramente algebraica:
