...
El carbono y el silicio convergen en unas decenas de iteraciones sin ninguna ayuda. En el germanio y el estaño el apantallamiento de un core extenso reduce la separación energética entre el orbital de valencia $np$ y los virtuales de su misma simetría, y un ciclo con poca separación entre ocupados y virtuales es propenso a oscilar. Una variación pequeña de la matriz de densidad $\bm{P}$ puede entonces reordenar los autovalores del bloque de valencia $\bm{F}_p$ de una iteración a la siguiente.

El riesgo concreto es el cruce de niveles. Si en alguna iteración un virtual del bloque $p$ queda por debajo del orbital de valencia, el principio de construcción ocupa el autovector equivocado y la densidad de la iteración siguiente se forma en el subespacio incorrecto. El ciclo puede entonces alternar entre dos soluciones sin converger a ninguna.

Para estabilizar el ciclo en estos dos elementos empleamos el desplazamiento de nivel (\textit{level shifting}) de Saunders y Hillier \cite{Saunders1973}, que eleva las energías del espacio virtual y deja intacto el espacio ocupado.
...
El desplazamiento de nivel estabiliza el ciclo pero no lo acelera: al ampliar la brecha artificialmente también reduce el paso efectivo de cada iteración, y mantenido durante todo el ciclo lleva la convergencia del germanio y del estaño a cerca de doscientas cincuenta iteraciones (Cuadro \ref{tab:cdiis}). La técnica que resuelve el problema de fondo es la extrapolación de Pulay, que en lugar de amortiguar la iteración construye, a partir del historial de matrices de Fock, la combinación lineal que más se acerca al punto fijo. Su variante moderna toma como medida del error el conmutador entre la matriz de Fock y la de densidad, y por eso se la conoce como C-DIIS.
