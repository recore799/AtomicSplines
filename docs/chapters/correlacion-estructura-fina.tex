...
\todo{a ver que le pongo al capitulo para que incluya Vpol}
...
La inclusión de $V_{\text{pol}}$ altera la estructura orbital de los propios orbitales de valencia. El efecto atractivo apantalla el pozo nuclear profundo, permitiendo que la distribución de probabilidad se relaje y se recorra ligeramente hacia el exterior. Esta leve relajación reduce de forma importante el \textbf{momento radial de orden inverso cúbico}:
\begin{equation}
    \left\langle \frac{1}{r^3} \right\rangle_{nl} = \int_0^\infty P_{nl}^2(r) \frac{1}{r^3} \dd{r}.
\end{equation}
Dado que el parámetro de estructura fina magnético $\zeta_{nl}$ (que gobierna el Hamiltoniano de Breit-Pauli) es un observable que escala asintóticamente de manera directa con $\langle 1/r^3 \rangle$, la omisión de la polarización del core condena al modelo de Hartree-Fock puro a sobreestimar este momento inverso. Al no poseer $V_{\text{pol}}$, el campo central atrae excesivamente a la nube de valencia hacia $r \to 0$, incrementando la penetración radial y produciendo un acoplamiento espín-órbita anómalamente alto. Este comportamiento subraya la necesidad de incorporar correcciones de correlación adicionales para modelar adecuadamente átomos pesados como el Germanio y el Estaño.
