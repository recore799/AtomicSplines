...
@load "oganesson_results_R20.0.jld2" orbitals E_total R_max
...
function plot_oganesson(basis::BSplineBasis, orbitals::Vector{Orbital}, R_max::Float64)
    ...
    labels = ["1s", "2s", "3s", "4s", "5s", "6s", "7s", "2p", "3p", "4p", "5p", "6p", "7p", "3d", "4d", "5d" , "6d", "4f", "5f"]
    
    # Initialize the plot with the custom X-axis ticks
    p = plot(title="Hartree-Fock Radial Wavefunctions of Oganesson (non-relativistic)", 
             xlabel="r (a.u.)", 
             ylabel="P(r)", 
    ...
    display(p)
    savefig(p, "oganesson_orbitals.pdf")
end
...
function plot_oganesson_density(basis::BSplineBasis, orbitals::Vector{Orbital}, R_max::Float64)
    ...
    # 4. Create the plot
    p = plot(title="Total Radial Electron Density of Oganesson (non-relativistic)", 
             xlabel="r (a.u.)", 
             ylabel="D(r)", 
             legend=false, 
             size=(850, 500),
    ...
    savefig(p, "oganesson_total_density.pdf")
end
...
plot_oganesson(basis, orbitals, R_max)
plot_oganesson_density(basis, orbitals, R_max)
