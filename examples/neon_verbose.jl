...
function solve_neon(R_max; verbose::Bool=true)
    ...
    if verbose
        ...
        @printf("%-4s | %-14s | %-10s | %-9s | %-9s | %-9s | %-8s\n", 
                "Iter", "E_total (Ha)", "Delta E", "ε_1s", "ε_2s", "ε_2p", "Time (s)")
        println("-"^78)
    end
    ...
    for iter in 1:60
        ...
        if verbose
            # Added the elapsed time to the printout
            @printf("%-4d | %14.8f | %10.2e | %9.4f | %9.4f | %9.4f | %8.4f\n", 
                    iter, E_total, delta, orbitals[1].energy, orbitals[2].energy, orbitals[3].energy, elapsed)
        end
