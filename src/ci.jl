...
function extract_virtuals(evals::Vector{Float64}, evecs::Matrix{Float64}, l::Int, num_occupied::Int, N_virt::Int, active_idx::UnitRange{Int}, n_splines::Int, ws)
    virtuals = Orbital[]
    
    # We skip the eigenvectors that are already holding electrons
    start_idx = num_occupied + 1
    end_idx = min(start_idx + N_virt - 1, length(evals))
    
    for i in start_idx:end_idx
        # Assign a pseudo-principal quantum number purely for bookkeeping
        # e.g., if l=1 (p-wave) and it's the 1st virtual, n = 2.
...
function compute_Rk(ws::SolverWorkspace{K}, a::Orbital, b::Orbital, c::Orbital, d::Orbital, k_mult::Int) where {K}
    ...
    # Pre-allocate or pull from workspace buffers to avoid allocations
    source = zeros(Float64, n)
    y_k    = zeros(Float64, n)
    
    # --- 1. Build Source Vector from Orbitals b and d ---
    for i in 1:(length(ws.basis.knots)-1)
        ...
        for k_idx in 1:K
            ...
            val = 0.0
            for alpha in 1:K
                # Assuming W tensor structure: W[k_idx, alpha, beta] integrates B_k * B_alpha * B_beta
                for beta in 1:K
